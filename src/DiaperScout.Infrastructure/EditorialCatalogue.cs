...
internal sealed class CanonicalCatalogue(DiaperScoutDbContext db) : ICanonicalCatalogue
{
    ...
    public async Task<CanonicalProductReceipt> CreateProductAsync(AuthenticatedUser actor, CreateCanonicalProduct command, CancellationToken cancellationToken = default)
    {
        Validate(command);
        ...
        var manufacturerExists = await db.Manufacturers.AnyAsync(manufacturer => manufacturer.Id == command.ManufacturerId, cancellationToken);
        if (!manufacturerExists) throw new CatalogueValidationException("manufacturerId", "The manufacturer was not found.");
        if (command.BrandId is { } brandId && !await db.Brands.AnyAsync(brand => brand.Id == brandId && brand.ManufacturerId == command.ManufacturerId, cancellationToken))
            throw new CatalogueValidationException("brandId", "The brand was not found for the selected manufacturer.");
        if (await db.Products.AnyAsync(product => product.Slug == command.ProductSlug, cancellationToken))
            throw new CatalogueValidationException("productSlug", "A product with this slug already exists.");
        if (gtin is not null &&
            await db.ProductIdentifiers.AnyAsync(identifier => identifier.Type == IdentifierType.Gtin && identifier.Value == gtin, cancellationToken))
            throw new CatalogueValidationException("gtin", "This GTIN is already assigned to a pack type.");

        var product = new Product(command.ManufacturerId, command.BrandId, command.ProductName.Trim(), command.ProductSlug, command.ProductType, command.Status);
        var variant = new ProductVariant(product.Id, command.VariantName.Trim(), command.BackingType);
        var size = new SizeVariant(variant.Id, command.ManufacturerSize.Trim(), command.WaistMinimumCm, command.WaistMaximumCm);
        var pack = new PackType(size.Id, command.QuantityPerPack, command.PackagingType);

        db.AddRange(product, variant, size, pack);

        ProductIdentifier? identifier = null;
        if (gtin is not null)
        {
            identifier = new ProductIdentifier(pack.Id, IdentifierType.Gtin, gtin);
        ...
        var audit = new CatalogueAuditRecord(
            ...
            JsonSerializer.Serialize(
                identifier is null
                    ? new[] { product.Id, variant.Id, size.Id, pack.Id }
                    : new[] { product.Id, variant.Id, size.Id, pack.Id, identifier.Id }),
            command.SourceSummary.Trim(),
            JsonSerializer.Serialize(command.SourceReferences.Select(reference => reference.Trim())),
        ...
        return new CanonicalProductReceipt(product.Id, variant.Id, size.Id, pack.Id, audit.Id, gtin);
    }

    private static void Validate(CreateCanonicalProduct command)
    {
        Required(command.ProductName, "productName"); Required(command.ProductSlug, "productSlug"); Required(command.VariantName, "variantName"); Required(command.ManufacturerSize, "manufacturerSize"); Required(command.SourceSummary, "sourceSummary"); Required(command.EditorialRationale, "editorialRationale");
        if (!SlugPattern.IsMatch(command.ProductSlug)) throw new CatalogueValidationException("productSlug", "Product slug must contain lowercase letters, digits, and single hyphens only.");
        if (!string.IsNullOrWhiteSpace(command.Gtin) &&
            !GtinPattern.IsMatch(command.Gtin.Replace(" ", string.Empty).Replace("-", string.Empty)))
            throw new CatalogueValidationException("gtin", "GTIN must contain 8 to 14 digits.");
        if (!Enum.IsDefined(command.ProductType) || !Enum.IsDefined(command.Status) || !Enum.IsDefined(command.BackingType) || !Enum.IsDefined(command.PackagingType)) throw new CatalogueValidationException("catalogue", "One or more catalogue classifications are invalid.");
        if (command.QuantityPerPack <= 0) throw new CatalogueValidationException("quantityPerPack", "Quantity per pack must be greater than zero.");
        if (command.WaistMinimumCm is not null && command.WaistMaximumCm is not null && command.WaistMinimumCm > command.WaistMaximumCm) throw new CatalogueValidationException("waist", "Waist minimum cannot exceed waist maximum.");
        if (command.SourceReferences is null || command.SourceReferences.Count == 0 || command.SourceReferences.Any(string.IsNullOrWhiteSpace)) throw new CatalogueValidationException("sourceReferences", "At least one source reference is required.");
    }
